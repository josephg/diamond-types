...
use crate::list::old_merge::{DocRangeIndex, M2Tracker, SpaceIndex};
#[cfg(feature = "ops_to_old")]
use crate::list::old_merge::to_old::OldCRDTOp;
use crate::list::old_merge::yjsspan::{INSERTED, NOT_INSERTED_YET, YjsSpan};
...
use crate::list::old_merge::dot::{DotColor, name_of};
#[cfg(feature = "dot_export")]
use crate::list::old_merge::dot::DotColor::*;
...
impl M2Tracker {
    ...
    fn apply_range(&mut self, oplog: &ListOpLog, range: DTRange, mut to: Option<&mut ListBranch>) {
        ...
        if let Some(to) = to.as_deref_mut() {
            to.version.advance(&oplog.cg.graph, range);
        }
        ...
        match op.kind {
            ...
            ListOpKind::Del => {
                ...
                if !is_underwater(target.start) {
                    ...
                    debug_assert!(oplog.cg.graph.version_contains_time(&[lv_start], target.start));
                }
    ...
    fn walk(&mut self, oplog: &ListOpLog, start_at: Frontier, rev_spans: &[DTRange], mut apply_to: Option<&mut ListBranch>) -> Frontier {
        let mut walker = SpanningTreeWalker::new(&oplog.cg.graph, rev_spans, start_at);
...
impl<'a> TransformedOpsIter<'a> {
    fn new(oplog: &'a ListOpLog, from_frontier: FrontierRef, merge_frontier: FrontierRef) -> Self {
        ...
        let common_ancestor = oplog.cg.graph.find_conflicting(from_frontier, merge_frontier, |span, flag| {
...
impl<'a> Iterator for TransformedOpsIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if self.ff_mode {
            ...
            let txn = self.oplog.cg.graph.0.find_packed(span.start);
            ...
            if can_ff {
                ...
                if self.did_ff {
                    ...
                    self.common_ancestor = self.oplog.cg.graph.find_conflicting(self.next_frontier.as_ref(), self.merge_frontier.as_ref(), |span, flag| {
        ...
        let (tracker, walker) = match self.phase2.as_mut() {
            ...
            None => {
                ...
                let walker = SpanningTreeWalker::new(&self.oplog.cg.graph, &self.new_ops, frontier);
        ...
        let (mut pair, op_iter) = loop {
            ...
            self.next_frontier.advance(&self.oplog.cg.graph, walk.consume);
            self.op_iter = Some(self.oplog.iter_metrics_range(walk.consume).into());
        };

        // Ok, try to consume as much as we can from pair.
        let span = self.oplog.lv_span_to_agent_span(pair.span());
...
impl ListBranch {
    ...
    // TODO: Remove me!
    #[allow(unused)]
    fn merge_old(&mut self, oplog: &ListOpLog, merge_frontier: FrontierRef) {
        // The strategy here looks like this:
        // We have some set of new changes to merge with a unified set of parents.
        // 1. Find the parent set of the spans to merge
        // 2. Generate the conflict set, and make a tracker for it (by iterating all the conflicting
        ...
        let mut common_ancestor = oplog.cg.graph.find_conflicting(self.version.as_ref(), merge_frontier, |span, flag| {
            // Note we'll be visiting these operations in reverse order.
        ...
        if ALLOW_FF {
            loop {
                if let Some(span) = new_ops.last() {
                    let txn = oplog.cg.graph.0.find_packed(span.start);
                    let can_ff = txn.with_parents(span.start, |parents| {
                        ...
                        local_frontier_eq(self.version.as_ref(), parents)
                    });
        ...
        if did_ff {
            ...
            conflict_ops.clear();
            shared_size = 0;
            common_ancestor = oplog.cg.graph.find_conflicting(self.version.as_ref(), merge_frontier, |span, flag| {
                if flag == DiffFlag::Shared {
                    shared_size += span.len();
                    shared_ranges += 1;
                }
...
mod test {
    ...
    fn test_ff() {
        ...
        assert_eq!(list.branch.version.as_ref(), &[2]);
