...
impl<'a> SpanningTreeWalker<'a> {
    pub(crate) fn new_all(parents: &'a Parents) -> Self {
        let mut spans: SmallVec<[DTRange; 4]> = smallvec![];
        if parents.get_next_time() > 0 {
            // Kinda gross. Only add the span if the document isn't empty.
            spans.push((0..parents.get_next_time()).into());
        }
        // spans.reverse(); // Unneeded - there's 0 or 1 item in the spans list.

        Self::new(parents, &spans, Frontier::root())
    }

    pub(crate) fn new_between(parents: &'a Parents, from: &[LV], to: &[LV]) -> Self {
        let (_a, txns) = parents.diff(from, to);
        // _a might always be empty.
        debug_assert!(_a.is_empty());
        SpanningTreeWalker::new(parents, &txns, from.into())
    }
...
mod test {
    ...
    fn iter_span_for_empty_doc() {
        ...
        let walk = SpanningTreeWalker::new_all(&history).collect::<Vec<_>>();
