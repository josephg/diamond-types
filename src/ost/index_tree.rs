...
mod test {
    ...
    fn expect_eq<'a, T: Default + Copy + Debug + Eq + PartialEq + 'a>(tree: &IndexTree<T>, vals: impl IntoIterator<Item = &'a RleDRun<T>>) {
        expect_eq_2(tree, vals.into_iter().copied())
    }

    fn expect_eq_2<T: Default + Copy + Debug + Eq + PartialEq>(tree: &IndexTree<T>, vals: impl IntoIterator<Item = RleDRun<T>>) {
        tree.dbg_check();
    ...
    fn overlapping_sets() {
        ...
        expect_eq(&tree, &[RleDRun::new(5..10, Some(A))]);
        // assert_eq!(tree.to_vec(), &[((5..10).into(), Some(A))]);
        tree.dbg_check();
        tree.set_range((5..11).into(), Some(B), true);
        expect_eq(&tree, &[RleDRun::new(5..11, Some(B))]);
        tree.dbg_check();

        tree.set_range((5..10).into(), Some(A), true);
        expect_eq(&tree, &[
            RleDRun::new(5..10, Some(A)),
            RleDRun::new(10..11, Some(B)),
        ]);
    ...
    fn split_values() {
        ...
        tree.dbg_check();
        expect_eq(&tree, &[
            RleDRun::new(10..12, Some(A)),
            RleDRun::new(12..15, Some(B)),
            RleDRun::new(15..20, Some(A)),
        ]);
    }
    ...
    fn set_inserts_1() {
        ...
        expect_eq(&tree, &[RleDRun::new(5..10, Some(B))]);
        tree.dbg_check();
        ...
        expect_eq(&tree, &[
            RleDRun::new(5..10, Some(B)),
            // ((5..10).into(), Some(B)),
            // ((10..15).into(), None),
            RleDRun::new(15..20, Some(C)),
        ]);
        tree.dbg_check();
    ...
    fn set_inserts_2() {
        ...
        dbg!(&tree);
        expect_eq(&tree, &[
            RleDRun::new(1..3, Some(B)),
            RleDRun::new(3..8, Some(C)),
            RleDRun::new(8..10, Some(A)),
        ]);
        tree.dbg_check();
    ...
    fn split_leaf() {
        ...
        expect_eq(&tree, &[
            RleDRun::new(1..2, Some(A)),
            RleDRun::new(2..3, Some(B)),
            RleDRun::new(3..4, Some(A)),
            RleDRun::new(4..5, Some(B)),
            RleDRun::new(5..6, Some(A)),
        ]);
    }
    ...
    fn fuzz(seed: u64, verbose: bool) {
        ...
        check_tree.replace_range_at_offset(0, RleDRun::new(0..1000, None));
        ...
        for _i in 0..1000 {
            ...
            // }
            check_tree.replace_range_at_offset(start, RleDRun::new(start..start+len, Some(val)));
