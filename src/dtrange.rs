...
pub fn range_last(r: DTRange) -> usize {
    debug_assert!(r.end > r.start); // last is invalid for empty spans.
    r.end - 1
}

pub trait RangeHelpers {
    fn last(&self) -> usize;
    fn from_1(elem: usize) -> Self;
}
impl RangeHelpers for DTRange {
    fn last(&self) -> usize {
        range_last(*self)
    }

    fn from_1(elem: usize) -> Self {
        Self { start: elem, end: elem + 1 }
    }
}
