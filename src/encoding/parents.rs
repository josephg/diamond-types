...
pub(crate) fn write_parents_raw(ids_out: &mut impl ExtendFromSlice, entries_out: &mut impl ExtendFromSlice,
                                parents: &[LV], next_output_time: LV, write_map: &mut WriteMap, aa: &AgentAssignment) {
    ...
    //   2. A "foreign change". Mark the mapped user agent and the seq.

    // To express this, each item has 3 bits mixed in:
    // 1. has_more (are there more entries in this list of parents)
    ...
    // A foreign change is (has_more, true, true) with mapped agent ID 1+mapped.
    ...
    if parents.is_empty() {
        ...
        // empty.
        write_parent_diff(entries_out, 0, false, true);
    } else {
        ...
        while let Some(&p) = iter.next() {
            ...
            if let Some((map, offset)) = write_map.txn_map.find_with_offset(p) {
                ...
                write_parent_diff(entries_out, next_output_time - mapped_parent, has_more, false);
            } else {
                // Foreign change
                ...
                let mapped_agent = write_map.map_and_store(item.0, &aa.client_data, ids_out);
                ...
                // here.
                write_parent_diff(entries_out, mapped_agent as usize + 1, has_more, true);

                // And the sequence number.
                push_usize(entries_out, item.1);
            }
        }
    }
}
        ...
        let parent = if !is_foreign {
            ...
            let agent = match n {
                ...
                n => {
                    ...
                    let mapped_agent = n - 1;
                    let Some(entry) = read_map.agent_map.get(mapped_agent) else {
...
mod test {
    ...
    //     let seph = aa.get_or_create_agent_id_from_str("seph");
    ...
    //     let george = aa_out.get_or_create_agent_id_from_str("george");
    ...
    //
    //     // 2. Foreign agent
    //     let seph = aa_out.get_or_create_agent_id_from_str("seph");
